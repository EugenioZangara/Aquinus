...
class CalificacionesForm(forms.ModelForm):
    ...
    def __init__(self, *args, fechas_periodo=None, **kwargs):
        ...
        # Guardamos fechas_periodo como un atributo de la instancia del formulario
        self.fechas_periodo = fechas_periodo
    ...
    def clean_fecha_examen(self):
        ...
        # Asegurarse de que fechas_periodo esté disponible para la validación
        if fecha_examen and self.fechas_periodo:
            ...
            if not (fecha_inicio <= fecha_examen <= fecha_fin):
                raise forms.ValidationError(f"La fecha debe estar entre {fecha_inicio} y {fecha_fin}.")
        return fecha_examen
